...
const Stark = () => {
  ...
  const handleOk = async () => {
    try {
      ...
      if (
        values.address.length !== 66 &&
        values.address.length !== 64 &&
        values.address.length !== 65
      ) {
        notification.error(
          {
            message: "错误",
      ...
      if (index !== -1) {
        ...
        getStarkInfo(values.address).then(
          ({ wallet_type, deployed_at_timestamp, stark_id }) => {
            updatedData[index] = {
              ...updatedData[index],
        ...
        getStarkBalances(values.address).then(
          ({ eth_balance, usdc_balance, usdt_balance, dai_balance }) => {
            updatedData[index] = {
              ...updatedData[index],
        ...
        getStarkActivity(values.address).then(
          ({ dayActivity, weekActivity, monthActivity }) => {
            updatedData[index] = {
              ...updatedData[index],
    ...
    try {
      ...
      for (let key of selectedKeys) {
        ...
        if (index !== -1) {
          ...
          setData([...newData]);
          promisesQueue.push(() => {
            return getStarkTx(item.address).then(
              ({ tx, stark_latest_tx_time, stark_timestamps }) => {
          ...
          // })})
          promisesQueue.push(() => {
            return getStarkERC20(item.address).then(
              ({ eth_balance, usdc_balance, usdt_balance, dai_balance }) => {
                item.stark_eth_balance = eth_balance;
                item.stark_usdc_balance = usdc_balance;
                item.stark_usdt_balance = usdt_balance;
      ...
      localStorage.setItem(
        "stark_timestamps",
        JSON.stringify(stark_timestamps)
      );
      for (let key of selectedKeys) {
        const index = newData.findIndex((item) => item.key === key);
        if (index !== -1) {
          ...
          if (item.wallet_type === "Error" || item.wallet_type === undefined) {
            getStarkInfo(item.address).then(
              ({ wallet_type, deployed_at_timestamp, stark_id }) => {
                item.wallet_type = wallet_type;
                item.create_time = deployed_at_timestamp;
                item.stark_id = stark_id;
          ...
          getStarkActivity(item.address).then(
            ({ dayActivity, weekActivity, monthActivity }) => {
              item.dayActivity = dayActivity;
              item.weekActivity = weekActivity;
              item.monthActivity = monthActivity;
        ...
        },
        {
          ...
          render: (text, record) => {
            ...
            const normalizedValue =
              (logarithmValue - minValue) / (maxValue - minValue);

            // 计算透明度
            const minOpacity = 0.1; // 最小透明度
            const maxOpacity = 1; // 最大透明度
            const opacity =
              normalizedValue * (maxOpacity - minOpacity) + minOpacity;
        ...
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              align: "center",
              sorter: (a, b) =>
                a.stark_exchange_amount - b.stark_exchange_amount,
              render: (text, record) => {
                if (text === null) {
                  return <Spin />;
                }
        ...
        <Spin spinning={tableLoading}>
          <Table
            ...
            summary={(pageData) => {
              ...
              const emptyCells = Array(15)
                .fill()
                .map((_, index) => (
                  <Table.Summary.Cell key={index} index={index + 10} />
                ));
                ...
                <>
                  <Table.Summary.Row>
                    ...
                    <Table.Summary.Cell index={6}>
                      {starkEthBalance.toFixed(4)}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={7}>
                      {starkUsdcBalance.toFixed(2)}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={8}>
                      {starkUsdtBalance.toFixed(2)}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={9}>
                      {starkDaiBalance.toFixed(2)}
                    </Table.Summary.Cell>
                    {emptyCells}
                  </Table.Summary.Row>
                </>
